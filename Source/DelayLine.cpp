...
{
	// We can switch this out with the private methods showing different ways of generating the delay audio
	return calculateHermiteInterpolationRead(delayInSamples);
}

float DelayLine::calculateSimpleRead(float delayInSamples) const noexcept
{
	/*
		Note: Set paddedLength = maxLengthInSamples + 1 in setMaximumDelayInSamples()
	*/
...
float DelayLine::calculateLinearInterpolationRead(float delayInSamples) const noexcept
{
	/*
		Note: Set paddedLength = maxLengthInSamples + 1 in setMaximumDelayInSamples()
	*/
...
float DelayLine::calculateHermiteInterpolationRead(float delayInSamples) const noexcept
{
	/*
		Note: Set paddedLength = maxLengthInSamples + 2 in setMaximumDelayInSamples()
	*/
